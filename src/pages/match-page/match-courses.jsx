...
import { fetchGlobalMatchData } from '../../reducers/match-reducer'
...
const MatchCourses = ({ user, defaultUrl }) => {
  ...
  const formData = useSelector((state) => state.match)
  React.useEffect(() => {
    dispatch(fetchGlobalMatchData())
            ...
            <div className="row">
              <div className="col">
                ...
                <select className="form-select form-select-sm" id="course_name" name="course_name" required="">
                  <option value="">{ formData.loading ? 'Loading...' : '-------------' }</option>
                  {
